...
namespace {
    ...
    JSON parse_object(const string& str, size_t& offset)
    {
        ...
        while (true) {
            ...
            consume_ws(str, offset);
            if (str[offset] != ':') {
                ESP_LOGE(TAG, "Error: Object: Expected colon, found '%c'", str[offset]);
                break;
            }
            consume_ws(str, ++offset);
            ...
            if (str[offset] == ',') {
                ...
                ESP_LOGE(TAG, "ERROR: Object: Expected comma, found '%c'", str[offset]);
                break;
            }
        }
    ...
    JSON parse_array(const string& str, size_t& offset)
    {
        ...
        while (true) {
            ...
            if (str[offset] == ',') {
                ...
                ESP_LOGE(TAG, "ERROR: Array: Expected ',' or ']', found '%c'", str[offset]);
                return JSON::Make(JSON::Class::Array);
            }
        }
                ...
                case 'u': {
                    ...
                    for (unsigned i = 1; i <= 4; ++i) {
                        ...
                        if ((c >= '0' && c <= '9') || (c >= 'a' && c <= 'f') || (c >= 'A' && c <= 'F'))
                            ...
                            ESP_LOGE(TAG, "ERROR: String: Expected hex character in unicode escape, found '%c'", c);
        ...
        if (c == 'E' || c == 'e') {
            ...
            while (true) {
                ...
                if (c >= '0' && c <= '9')
                    ...
                    ESP_LOGE(TAG, "ERROR: Number: Expected a number for exponent, found '%c'", c);
            ...
            ESP_LOGE(TAG, "ERROR: Number: unexpected character '%c'", c);
            return (JSON::Make(JSON::Class::Null));
        }
        --offset;
    ...
    JSON parse_bool(const string& str, size_t& offset)
    {
        ...
        if (str.substr(offset, 4) == "true")
            ...
            ESP_LOGE(TAG, "ERROR: Bool: Expected 'true' or 'false', found '%s'", str.substr(offset, 5));
            return JSON::Make(JSON::Class::Null);
        }
    ...
    JSON parse_null(const string& str, size_t& offset)
    {
        ...
        if (str.substr(offset, 4) != "null") {
            ESP_LOGE(TAG, "ERROR: Null: Expected 'null', found '%s'", str.substr(offset, 4));
            return JSON::Make(JSON::Class::Null);
        }
        offset += 4;
        return Null;
    ...
    JSON parse_next(const string& str, size_t& offset)
    {
        ...
        ESP_LOGE(TAG, "ERROR: Parse: Unknown starting character '%c'", value);
        return JSON();
    }
}
